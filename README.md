...
A Firefox browser extension that transforms the GitHub interface with a custom theme inspired by Serial Experiments Lain.

## Features

*   **Lain-Wired Styled Color Palette:** Dark, brownish-red backgrounds, pale eerie blue accents, and subtle off-white tones.
...
*   **Custom Background Support:** Easily integrate your favorite Lain-themed background image.
...
4.  **Load Temporary Add-on:**
    ...
    *   Navigate to the directory where you have the Lain-Wired folder.
    *   Select any file *inside* the folder (e.g., `manifest.json` or `content.js`). Firefox will automatically detect the extension.
5.  **Verify Installation:**
    *   The "Lain-Wired GitHub Theme" should now appear in your list of temporary add-ons.
6.  **Test the Theme:**
    *   Open a new tab and navigate to `github.com`.
    *   You should see the Lain-Wired-themed colors and styling applied to the GitHub interface.

**Note:** Temporary add-ons are removed when Firefox is closed. To keep the extension permanently, you would need to install it using Firefox extension store page for Lain-Wired: [Soon to be added, please fix.].
...
2.  **Save the Image:** Download the image and save it as `lain-background.png` inside the icons directory.
3.  **Reload Extension:** Go to `about:debugging#/runtime/this-firefox` in Firefox, find "Lain-Wired", and click "Reload".
4.  **Refresh GitHub:** Refresh any open GitHub pages to see your new background.

## Contributing

Feel free to modify the `lain.css` file to further customize the theme to your liking. Experiment with colors, fonts, and element styles!
